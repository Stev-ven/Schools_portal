		...
		function resendActivationCode(event){
			var dis = Main.getElementById("resend_btn");
			Main.resendActivationCode({
				...
				success: function(data){
					if(Main.isStringifiedJSON(data)){
						...
						Main.ManageMessageBox({
							target : "#resend_activation_log",
							parent : "#resend_activation_log",
							message : pData.data,
							state : err,
		...
		var sbtn = document.getElementById("resend_btn"), ebtn = document.getElementsByClassName("emp-inp"), ebtnLength = document.getElementsByClassName("emp-inp").length;
		sbtn.addEventListener("click", resendActivationCode, false);
