...
const BASE_URL = __GLOBALS__.DOMAIN;
...
function loadPage(task, form, view, isNewTab, callback) {
    let data = "";
    isNewTab = (typeof isNewTab !== "undefined" && typeof isNewTab !== null) ? isNewTab : "no";

    if (isNewTab === "yes")
        data = "&task=" + task + "&viewType=" + view;
    else
        data = $(form).serialize() + "&task=" + task + "&viewType=" + view + ($.trim($(form).attr("data-advance-filter-form")) != "" ? "&" + $($(form).attr("data-advance-filter-form")).serialize() : "");
    
    $.ajax({
        type: "POST",
        url: BASE_URL + 'route.php',
        data: data,
